<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      <el-form-item label="农资类别名称" prop="materialTypeName">
        <el-input
    ...
    <el-row :gutter="10" class="mb8">
      ...
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['agriculture:materialType:export']"
        >导出</el-button>
      </el-col>
...
<script>
import { listType, getType, delType, addType, updateType } from "@/api/system/materialtype"
...
export default {
  ...
  methods: {
    ...
    handleSelectionChange(selection) {
      this.ids = selection.map(item => item.materialTypeId)
    ...
    handleUpdate(row) {
      ...
      getType(materialTypeId).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除农资类别编号为"' + materialTypeIds + '"的数据项？').then(function() {
        return delType(materialTypeIds)
      }).then(() => {
        this.getList()
